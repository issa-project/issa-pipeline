...
class ConceptDictionaryGenerator:
    """ 
    Generate dictionary file from a SKOS endpoint
    """
    def __init__(self, name, 
                 endpoint, graph, skosXL = False,
                 language = 'en',
                 output_dir = '.'):
        """
        name: name of the dictionary
        endpoint: SPARQL endpoint where to create dictionary from
        graph: graph URI where to create dictionary from
        skosXL: if True, use SKOS-XL labels
        language: language of the labels
        output_dir: output directory for the dictionary TSV file
        """
...
class ConceptAnnotator:
    """
    Annotate text with concepts from a dictionary file
    """
...
class ConceptAnnotatorCached(ConceptAnnotator):
    """
        Initialise Concept annotator and pickle it for later use 
    ...
    def __init__(self, dictionary_file, language='en', cache_dir='.'):
        ...
        dictionary_file: path to the dictionary file
        language: language of the dictionary
        cache_dir: directory where to store the cached file
        """
        file_name = os.path.splitext(dictionary_file)[0]
        self.cached_file = os.path.join(cache_dir, f'{file_name}-concept-recognizer.pkl')
        ...
        if not os.path.exists(self.cached_file):
            ...
            print( 'concept recognizer serialized')

        else:
            # deserialize
            with open(self.cached_file, 'rb') as f:
                self.concept_recognizer = pickle.load(f)
            print( 'concept recognizer deserialized')

    # static function to check if the cashed annotator file is available
    @staticmethod
    def is_cached(dictionary_name, language, cache_dir='.'):
        """
        Check if the cashed annotator file is available by its name and language
        ...
        Returns:
            True if the cached file is available, False otherwise
        """
        cached_file = os.path.join(cache_dir, f'{dictionary_name}-{language}-concept-recognizer.pkl')
        print('cached file: ', cached_file, 'exists: ', os.path.exists(cached_file))
