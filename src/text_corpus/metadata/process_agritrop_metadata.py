...
logger = open_timestamp_logger(log_prefix= os.path.splitext(os.path.basename(__file__))[0], 
                               ...
                               first_line = 'Process Agritrop metadata...')
#%%
def remove_nans(df):
    """
    ...
    # Check pdf URLs and remove non-pdfs
    is_pdf_url = df.pdf_url.apply(lambda x: x.lower().endswith('.pdf'))
    
    logger.info('Dropping %d records with no pdf URL: ' , is_pdf_url[~is_pdf_url].shape[0])
...
def split_relations(df):
    """
    OAI api provide fieldd relations that have multiple meanings .
...
def detect_title_lang(df):
    ...
    df[['title_lang', 'title_lang_score']] = df.title.fillna('').apply(detect_lang_wrapper)
    
    if cfg.FILL_NOT_DETECTED_LANG:
        df.title_lang.fillna(cfg.LANGUAGE, inplace=True)
...
def detect_abstract_lang(df):
    ...
    if cfg.FILL_NOT_DETECTED_LANG:
        df.abstract_lang.fillna(cfg.LANGUAGE, inplace=True)
...
def get_live_descriptors_labels(row, language='en'):
    ...
    res = cfg.SPARQL_WRAPPER.sparql_to_dataframe(query)
...
def get_live_labels(df):
    ...
    agrovoc_res = df.apply(lambda r: get_live_descriptors_labels(r, cfg.LANGUAGE), axis=1)
